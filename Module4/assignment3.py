...
#df = df.loc[:,['bgr','wc','rc']]
#df.drop(['id', 'classification', 'rbc', 'pc', 'pcc', 'ba', 'htn', 'dm', 'cad', 'appet', 'pe', 'ane'], axis=1, level=None, inplace=True, errors='raise')
df.drop(['id', 'classification'], axis=1, level=None, inplace=True, errors='raise')
...
df[['wc','rc','pcv']] = df[['wc','rc','pcv']].apply(pd.to_numeric,errors='coerce')
