...
def Plot2DBoundary(model,DTrain, LTrain, DTest, LTest):
  ...
  for index in LTest.index:
    ...
    img = data.iloc[index,:].reshape(num_pixels, num_pixels)
...
for i in range(num_images):
  data.loc[i,:] = data.loc[i,:].reshape(num_pixels, num_pixels).T.reshape(-1)
...
face_labels=pd.read_csv('Datasets/face_labels.csv',header=None).T.squeeze()
#
...
data_train, data_test , label_train , label_test = train_test_split(data,face_labels,test_size = 0.15 , random_state=7)
...
if Test_PCA:
  ...
  #
    ...
    data_test=pca.transform(data_test)
      ...
      #
      # .. your code here ..
    iso = manifold.Isomap(n_neighbors=54, n_components=2)
    iso.fit(data_train)
